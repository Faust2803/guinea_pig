...
public class TonConnectHandler : MonoBehaviour
{
    ...
    public event Action<SendTransactionResult?, bool> OnTransactionSendingFinish = delegate{ };

    // main tonconnect instance, use it to work with tonconnect
    public TonConnect tonConnect {get; private set;}

    private List<WalletConfig> _walletConfigs = new List<WalletConfig>();
    public List<WalletConfig> WalletConfigs => _walletConfigs;

    public WalletConfig CurrentConnectedWallet { get; private set; }
    ...
    private async void Start()
    {
        ...
        GetWalletConfigs(WalletsListSource, (list) =>
        {
            _walletConfigs = list;

            if (_walletConfigs.Count > 0)
            {
                if (!tonConnect.IsConnected) return;

                Debug.Log($"App name: {tonConnect.Wallet.Device.AppName}");

                CurrentConnectedWallet = _walletConfigs.FirstOrDefault(wallet =>
                wallet.AppName.Equals(tonConnect.Wallet.Device.AppName, StringComparison.OrdinalIgnoreCase));

                Debug.Log($"Current wallet name: {CurrentConnectedWallet.AppName} + {CurrentConnectedWallet.UniversalUrl}");
            }
        });
    }

    public async Task CreateTonConnectInstance()
    {
    ...
    public async Task SendTransaction(string recipientAddress, double amount)
    {
        ...
        Coins tons = new(amount);
        Message[] sendTons =
        {
            new Message(receiver, tons),
        ...
        SendTransactionRequest transactionRequest = new SendTransactionRequest(sendTons, validUntil, CHAIN.MAINNET, tonConnect.Account.Address);
        ...
        try
        {
            ...
            Application.OpenURL(CurrentConnectedWallet.UniversalUrl);

            transactionResult = await tonConnect.SendTransaction(transactionRequest);

            //UnitonConnectLogger.Log($"Transaction successfully completed, Boc: {transactionResult.Value.Boc}");

            OnTransactionSendingFinish?.Invoke(transactionResult, true);
        }
        catch (WalletNotConnectedError connectionError)
        {
            //UnitonConnectLogger.LogError($"{connectionError.Message}");

            OnTransactionSendingFinish?.Invoke(transactionResult, false);
        }
